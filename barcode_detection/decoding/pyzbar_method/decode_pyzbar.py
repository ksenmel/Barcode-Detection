...
class DecodePyzbar(Decoder):
    ...
    def decode(self, images_dir: str, bounding_boxes_dir: str):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            imgs = Path(tmp_dir) / self.IMGS_DIR
            imgs.mkdir()
            ...
            crop_image(save_dir=str(imgs),images_dir=images_dir, bounding_boxes_path=bounding_boxes_dir)

            container = self.client.containers.run(
                self.DOCKER_IMAGE_NAME,
                volumes={
                    imgs: {"bind": self.DOCKER_IMG_BIND_PATH},
